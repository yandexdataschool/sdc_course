...
def visualize_binmgs(imgs, preds):
    preds = torch.where(preds > 0, torch.tensor(250), torch.tensor(0))
    imgs = torch.where(imgs > 0, torch.tensor(250), torch.tensor(0))
    return imgs.cpu(), preds.cpu()
